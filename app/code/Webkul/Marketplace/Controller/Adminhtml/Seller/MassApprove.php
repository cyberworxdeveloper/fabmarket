...
class MassApprove extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $item) {
            ...
            $userid=$seller['entity_id'];

            $salepatner= " SELECT * FROM `mg_marketplace_userdata` where seller_id=$userid";
             $data_sales= $connection->fetchAll($salepatner);

             $shoptitle=$data_sales[0]['shop_title'];
            $emailTempVariables['myvar1'] = $seller->getName();
            $emailTempVariables['myvar4'] = $shoptitle;
