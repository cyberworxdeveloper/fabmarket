...
class BecomesellerPost extends Action
{
    ...
    private function isExistingShopUrl()
    {
       ...
       $collection->addFieldToFilter('shop_title', $shoptitle);
    ...
    private function saveSellerData()
    {
        ...
        try {
            if ($status) {
                ...
                /*$emailTemplateVariables['myvar12'] = $customer->getShoptitle().' '.
                $emailTemplateVariables['myvar13'] = $customer->getProfileurl().' '.
                $emailTemplateVariables['myvar14'] = $customer->getSellerCountry().' '.*/
                $customer->getMiddlename().' '.$customer->getLastname();
                $emailTemplateVariables['myvar2'] = $this->backendUrl->getUrl(
                    'customer/index/edit',
                    ['id' => $customer->getId()]
                );
               $shoptitle = $this->getRequest()->getParam("shop_title");
                $emailTemplateVariables['myvar3'] = $this->_helper->getAdminName();
                $emailTemplateVariables['myvar55'] = $shoptitle;
                $emailTemplateVariables['myvar66'] = "Tiwari Alok";
