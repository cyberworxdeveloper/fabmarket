 ...
 *
 * @category    Cyberworx
 * @package     Cyberworx_CustomerAccount
 * @copyright   Copyright (c) 2018-2019 GiaPhuGroup Co., Ltd. All rights reserved. (http://www.giaphugroup.com/)
 * @license     https://www.giaphugroup.com/LICENSE.txt
 */

namespace Cyberworx\CustomerAccount\Controller\Customer\Ajax;
...
class Register extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if ($this->session->isLoggedIn()) {
            ...
            try {
                ...
                $is_seller= $this->getRequest()->getParam('is_seller');
            if($is_seller==1){
             $entity_id= $customer->getId();
             $shop_pan= $this->getRequest()->getParam('shop_pan');
              $shoptitle= $this->getRequest()->getParam('shoptitle');
               $shop_gst= $this->getRequest()->getParam('shop_gst');
               $company_locality= $this->getRequest()->getParam('company_locality');
               $shop_website= $this->getRequest()->getParam('shop_website');
               $contact_number = $this->getRequest()->getParam('contact_number');
              $this->updatesaller($entity_id,$shop_pan,$shop_gst,$company_locality ,$shop_website, $contact_number,$shoptitle);
	    }
                ...
                $response = [
                    'errors' => true,
                    'message' => __(
                        'Email id already exists with us.',
                        $url
                    )
                ];
    ...
    protected function updatesaller($entity_id,$shop_pan,$shop_gst,$shop_website,$company_locality,$contact_number,$shoptitle)
    {
   
    if ($entity_id != '') {

$url = 'http://3.6.21.143/ajaxpage/updatesaller.php';
$ch = curl_init($url);
$data = array(
    'entity_id' => $entity_id,
    'shop_pan' => $shop_pan,
...
curl_close($ch);
    ...
    protected function getSuccessMessage()
    {
        if ($this->addressHelper->isVatValidationEnabled()) {
			...
			$message = __('Thank you for registering with FAB Market.');
        }
        return $message;
    }
}
